...
> 待维护者衡量的审查发现。
>
> **审查之后的变化（2026-09-11）：** 本报告是 2026-08-18 的时点快照，以下
> 行为在审查之后已经落地，本报告中的相应陈述不再反映当前代码：
>
> - 第 4 节「请求日志不存 prompt / completion」——现在是可选的内容记录功能
>   （默认关闭，3/7/30 天保留，传输凭据排除在外，外部访问走只读 log key，
>   见 `docs/log-api.md`）。
> - 第 4 节「客户端 API key 只存 SHA-256，明文只显示一次」——key 现在同时
>   加密保存，管理员可以通过 `POST /api/keys/{id}/secret` 读回；SHA-256 哈希
>   仍是请求路径的认证依据。
> - 第 4 节「`X-Forwarded-For` 默认不信任」——`TRUST_PROXY_HEADERS` 现在默认
>   为开，面向反代后部署；`false` 恢复对端地址。
