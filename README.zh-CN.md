...
这些说明出现在请求日志里。**丢字段却没有说明就是 bug**——一个遍历全
...
这不是 passthrough 模式。每条请求仍然经过 canonical，保真度说明照样记录，用量照
样计量。
...
创建和管理 API Key。鉴权走 SHA-256 哈希，同时另存一份加密副本，所以你随时能在
列表里把自己的 Key 复制回来，不必因为忘了就删掉重建一遍限流和预算。每个 Key 展示使
