...
RBAC、图像/视频/音乐生成、独立的音频生成、WebRTC、向量数据库、RAG、agent，或者 MCP。

**允许实时语音的协议转换。** 包括 Gemini Live API 所需的双向 WebSocket、会话消息、实时音频
输入与音频输出、打断、转写和工具调用。它是协议能力，不是让 Polyglot 自己运行语音 agent；
不要把这个许可扩展成 WebRTC 或独立的音频生成产品。Live 的会话协议不同于现有的
`generateContent` HTTP/SSE；不能只把 Live 模型名注册到现有 Gemini codec，就宣称支持了实时
语音。客户端和上游的消息都必须按各自的协议处理，跨协议转换仍要经过 canonical 并报告保真度。
...
**已实现的实时能力要说清协议边界。** Gemini Live 客户端到 Gemini Live 上游的双向 WebSocket
会话已经支持实时音频；其它实时协议之间的转换尚未实现。普通 HTTP/SSE codec 的音频输入、
embeddings 和 token 计数仍在计划中，不能因为 Live 会话已支持音频就宣称它们也已实现。

在相应协议真正实现之前，音频必须保持在*被报告*的状态，绝不能换个形状转发出去。当前
`audio/*` 和 `video/*` 会被 `protocol.ClassifyMedia` 拒绝，因为没有这一条它们会掉进 "file" 分支，
然后作为一份上游会拒绝的文档发过去 —— 那就把"还没实现"变成了一次看起来毫不相干的失败。
